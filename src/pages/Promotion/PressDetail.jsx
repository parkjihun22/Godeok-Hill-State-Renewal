...
export default function PressDetail() {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        ...
        // Firestore 조회 실패 시 기본 보도자료로 이어서 표시합니다.
      }
  ...
  const seoPage = useMemo(() => {
    ...
    return {
      ...seoPages.press,
      path: detailPath,
      title: `${article.title} | 힐스테이트 고덕엘리스트 언론보도`,
      description:
        getArticleSummary(article) ||
        "힐스테이트 고덕엘리스트 언론보도 상세 페이지입니다.",
      image,
      ogType: "article",
      article: {
      ...
      <main className={styles.pressDatailWrap}>
        <article className={styles.pressDetailArticle}>
          <header className={styles.pressDetailHeader}>
            ...
            <h1 className={styles.pressDetailTitle}>{article.title}</h1>
